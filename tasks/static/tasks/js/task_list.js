...
function openTaskDrawer(taskId) {
  ...
  if (body) body.innerHTML = '<p class="text-sm text-gray-400 text-center py-10"><i class="fas fa-spinner fa-spin mr-2"></i>Loading...</p>';
...
// 🟢 Updated: Real image render karega agar url provide kiya gaya ho
function avatarHtml(name, size = 8, imageUrl = null) {
  const pxSize = size * 4; // tailwind sizing (w-8 = 32px)
  if (imageUrl) {
    return `<img src="${imageUrl}" alt="${name || ''}" class="w-${size} h-${size} rounded-full object-cover ring-1 ring-gray-200 shadow-xs flex-shrink-0">`;
  }
  return `<span class="w-${size} h-${size} rounded-full text-white text-xs flex items-center justify-center font-semibold flex-shrink-0 shadow-xs" style="background-color:${avatarColor(name)}">${initials(name)}</span>`;
}
...
function renderDrawer(taskId, t) {
  ...
  let managerName = '—';
  if (t.manager) {
    managerName = typeof t.manager === 'object' ? (t.manager.name || t.manager.username) : t.manager;
  }
  ...
  let html = `
    ...
    <div class="grid grid-cols-2 gap-4 text-sm mb-5 pb-5 border-b border-gray-100">
      <!-- ASSIGNED TO WITH IMAGE -->
      <div>
        <p class="text-[10px] font-bold text-gray-400 uppercase mb-1">Assigned To</p>
        <div class="flex items-center gap-2">
          ${avatarHtml(t.assigned_to, 8, t.assigned_to_pic)}
          <span class="font-semibold text-gray-800 text-xs">${t.assigned_to}</span>
        </div>
      </div>

      <!-- CREATED BY / MANAGER WITH IMAGE -->
      <div>
        <p class="text-[10px] font-bold text-gray-400 uppercase mb-1">Created By</p>
        <div class="flex items-center gap-2">
          ${avatarHtml(t.created_by, 8, t.created_by_pic)}
          <span class="font-semibold text-gray-800 text-xs">${t.created_by}</span>
        </div>
      </div>

      <div><p class="text-[10px] font-bold text-gray-400 uppercase">Department</p><p class="font-semibold text-gray-800 text-xs">${t.department || '—'}</p></div>
      <div><p class="text-[10px] font-bold text-gray-400 uppercase">Due Date</p><p class="font-semibold text-gray-800 text-xs">${t.due_date || '—'}</p></div>
      <div><p class="text-[10px] font-bold text-gray-400 uppercase">Milestone</p><p class="font-semibold text-gray-800 text-xs">${t.milestone || '—'}</p></div>
      <div><p class="text-[10px] font-bold text-gray-400 uppercase">Manager</p><p class="font-semibold text-gray-800 text-xs">${managerName}</p></div>
    </div>

    <div class="mb-5 pb-5 border-b border-gray-100">
      <p class="text-sm font-semibold text-gray-700 mb-2">Description</p>
      <div class="bg-gray-50 rounded-lg p-3 text-sm text-gray-600 border border-gray-100">${t.description || 'No description provided.'}</div>
    </div>`;
  ...
  html += `<div class="mb-5 pb-5 border-b border-gray-100">
    ...
    <div class="flex items-center justify-between py-1.5 px-2 rounded-lg bg-gray-50 mb-2">
      <div class="flex items-center gap-2">
        ${avatarHtml(t.assigned_to, 7, t.assigned_to_pic)}
        <span class="text-xs font-semibold text-gray-800">${t.assigned_to}</span>
      </div>
  ...
  t.team_members.forEach(m => {
    html += `<div class="flex items-center justify-between py-1.5 px-2">
      <div class="flex items-center gap-2">
        ${avatarHtml(m.username, 7, m.avatar)}
        <span class="text-xs text-gray-700">${m.name || m.username}</span>
      </div>
      ${t.can_manage ? `<button onclick="removeTeamMember(${taskId}, ${m.membership_id})" class="text-gray-400 hover:text-rose-600 text-xs transition"><i class="fas fa-times"></i></button>` : ''}
    </div>`;
  });

  if (t.colleagues && t.colleagues.length) {
    html += `<p class="text-[11px] text-gray-400 mt-3 mb-1">Suggested colleagues under ${managerName}:</p>`;
  }

  // Add Member Dropdown
  if (t.can_manage) {
    html += `<div class="flex gap-2 mt-2">
      <select id="addMemberSelect_${taskId}" class="border border-gray-200 rounded-lg px-2 py-1.5 text-xs flex-1 bg-white outline-none focus:border-indigo-500">
        <option value="">Choose colleague...</option>
        ${(t.colleagues || []).map(c => {
          const uId = c.id || c['user__id'];
          const uName = c.name || c['user__username'] || c.username;
          return `<option value="${uId}">${uName}</option>`;
        }).join('')}
      </select>
      <button onclick="addTeamMember(${taskId})" class="text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-lg transition shadow-xs">
